...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // On Vercel the key is server-side — hide badge completely
  const isVercel =
  ...
  if (medSel)
    medSel.addEventListener("change", () => UI.setBadge(medSel.value));

  if (document.getElementById("headerGenCount"))
    document.getElementById("headerGenCount").textContent =
...
function updateKeyBadge(hasKey) {
  ...
  badge.style.borderColor = hasKey
    ? "rgba(62,201,122,0.3)"
    : "rgba(232,160,32,0.3)";
...
function saveApiKey() {
  ...
  if (!key) {
    UI.toast("Please enter a valid Groq API key");
    return;
  }
...
async function generate() {
  ...
  try {
    const data = await API.generateScam({
...
function copyContent() {
  ...
  navigator.clipboard
    .writeText(State.currentContent.content || "")
    .then(() => UI.toast("Copied to clipboard ✓"));
}
