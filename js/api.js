// api.js — Groq API (llama-3.3-70b-versatile)
const API = {
  endpoint: 'https://api.groq.com/openai/v1/chat/completions',
  model: 'llama-3.3-70b-versatile',
  apiKey: '',  // Set your Groq API key here, or enter it in the UI

  getKey() {
    // Priority: config.js → sessionStorage (UI input) → hardcoded
    const configKey = (typeof CONFIG !== 'undefined' && CONFIG.GROQ_API_KEY && CONFIG.GROQ_API_KEY !== 'your_groq_api_key_here')
      ? CONFIG.GROQ_API_KEY : '';
    return configKey || this.apiKey || sessionStorage.getItem('groq_api_key') || '';
  },
  ...
  async generateScam({ scamType, medium, intensity, target }) {
    const mediumLabels = {
      phone: 'Phone Call Script (use [CALLER]: and [VICTIM]: prefixes for each line of dialogue)',
      sms: 'SMS / Text Message (short, punchy, with link or callback number)',
      email: 'Email (include realistic Subject line at very top)',
      voicemail: 'Voicemail Transcript (monologue only, caller leaving a recorded message)',
      chat: 'Chat / Direct Message thread (back and forth messages)',
      letter: 'Physical Mailed Letter (include fake letterhead, reference numbers, official-looking formatting)'
    };

    const targetDesc = {
      elderly: 'Target: elderly person (70s), less tech-savvy, possibly on fixed income, trusts authority figures',
      general: 'Target: general adult (30s-50s), moderate skepticism',
      professional: 'Target: working professional, higher income, somewhat skeptical but busy',
      student: 'Target: college student, limited income, worried about government/financial issues'
    };

    const intensityDesc = {
      low: 'Low pressure — friendly tone, soft urgency, no overt threats',
      medium: 'Medium pressure — clear urgency, consequences mentioned, time limit given',
